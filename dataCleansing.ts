function main(workbook: ExcelScript.Workbook) {
    ...
    let firstAssigneeSheetName = assigneeColumnValuesDeloiteOnly[0]
    // let firstAssigneeNewSheet = workbook.addWorksheet(firstAssigneeSheetName)

    const allRows = table.getRangeBetweenHeaderAndTotal().getValues()

    const filteredRowsOfFirstAssignee = allRows.filter(row => row[assigneeColumnIndex]=== assigneeColumnValuesDeloiteOnly[0])

    console.log(filteredRowsOfFirstAssignee)
    ...
    let newSheet = workbook.addWorksheet("Deloitte Only Emails")

    let headersOfNewSheet = table.getHeaderRowRange().getValues()[0];

    // Copy headersOfNewSheet
    newSheet.getRange("A1").getResizedRange(0, headersOfNewSheet.length - 1)
        .setValues([headersOfNewSheet]);

    // Paste filtered rows
    if (filteredRows.length > 0) {
        newSheet.getRange("A2").getResizedRange(filteredRows.length - 1, headersOfNewSheet.length - 1)
            .setValues(filteredRows);

        // Add table to new worksheet
